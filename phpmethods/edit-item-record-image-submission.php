...
include_once "function-writefootwearpage.php";
include_once "function-writeequipmentpage.php";
include_once "function-writefanclothingpage.php";
include_once "function-writeonsalepage.php";
$receiveditemid = $_POST['itemidbox'];
  ...
  if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
      ...
      // fetch info for the next part, updating the product tabs dynamically  
      $updateditemtype = $row["itemtype"];
      $updatedonsale = $row["onsale"];
    } 
  }
  else {
    echo "0 results<br>";
  ...
  // if a product item is added/amended, remake the php file
  if($updateditemtype === "clothing") {
    writeClothingPage();
  }
  else if($updateditemtype === "footwear") {
    writeFootwearPage();
  }
  else if($updateditemtype === "equipment") {
    writeEquipmentPage();
  }
  else if($updateditemtype === "fanclothing") {
    writeFanclothingPage();
  }
